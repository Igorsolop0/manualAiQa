...
Do not let this file turn into a dump of generic advice.

---

## Current Operational Truths

1. Nexus must stay behavior-aware and evidence-first.
2. Clawver is the UI executor; Nexus must not simulate Clawver work.
3. Stagehand is useful, but it must stay narrow and goal-based.
4. Pilot run artifacts and gates matter more than optimistic Slack prose.
5. Approval gate is real: no test execution before explicit user approval.
6. The current weak spot is exploratory task drift from Stagehand-first into oversized Playwright suites.
